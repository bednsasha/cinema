...
def create_uneven_hall():
    ...
    for name in ['2D', '3D']:
        ScreenType.objects.get_or_create(
    ...
    seats_data = [
        ...
        {'row': 5, 'seat': 3, 'seat_type': vip,
            'x': 110, 'y': 340},  # VIP место
        {'row': 5, 'seat': 4, 'seat_type': vip,
            'x': 150, 'y': 340},  # VIP место
    ...
    for data in seats_data:
        ...
        if created:
            print(
                f"  ✓ Ряд {data['row']}, место {data['seat']} - {data['seat_type'].display_name} (x={data['x']}, y={data['y']})")
